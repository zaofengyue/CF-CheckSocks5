	...
	<style>
		...
		* {
		...
		.token-toggle {
			...
			border: 1px solid rgba(255, 255, 255, 0.08);
			border-radius: 999px;
			background: linear-gradient(135deg, rgba(97, 219, 255, 0.14), rgba(45, 212, 191, 0.08));
			color: var(--muted);
			cursor: pointer;
			transition: transform 0.2s ease, background 0.28s ease, border-color 0.28s ease, color 0.28s ease, box-shadow 0.28s ease;
		}
		...
		.token-toggle:hover {
			...
			color: #ffffff;
			box-shadow: 0 4px 12px rgba(97, 219, 255, 0.15);
		}
		...
		.token-toggle.is-active {
			border-color: rgba(52, 211, 153, 0.45);
			color: var(--success);
			background: linear-gradient(135deg, rgba(52, 211, 153, 0.16), rgba(45, 212, 191, 0.1));
		}

		.token-toggle.is-active::after {
			content: '';
			position: absolute;
			top: 4px;
			right: 4px;
		...
		html[data-theme='light'] .token-toggle {
			border-color: rgba(84, 112, 139, 0.16);
			background: rgba(255, 255, 255, 0.65);
			color: #53708d;
		}

		html[data-theme='light'] .token-toggle:hover {
			border-color: rgba(97, 219, 255, 0.4);
			color: #0c1a2b;
		}

		html[data-theme='light'] .token-toggle.is-active {
		...
		.token-modal-backdrop.is-open .token-modal-card {
			transform: scale(1);
		}
		...
		.token-modal-input:focus {
			...
			box-shadow: 0 0 0 3px rgba(97, 219, 255, 0.15);
		}

		html[data-theme='light'] .token-modal-card {
			background: rgba(255, 255, 255, 0.96);
			border-color: rgba(84, 112, 139, 0.18);
		}

		html[data-theme='light'] .token-modal-input {
		...
		html[data-theme='light'] .token-modal-btn-cancel {
		...
		.token-modal-btn-save {
			background: linear-gradient(135deg, var(--accent), var(--accent-strong));
			color: #04131f;
			box-shadow: 0 4px 14px rgba(97, 219, 255, 0.25);
		}
...
<body>
		...
		<header class="site-header">
			<div class="brand">
				...
				<div class="brand-chip">
					...
					<button class="token-toggle" type="button" id="tokenToggleBtn" aria-label="配置访问 Token" title="配置访问 Token">
	...
	<script>
		...
		function getSavedToken() {
			try {
				return (localStorage.getItem('cf_proxy_token') || '').trim();
			} catch (e) {
				return '';
			}
		}

		function applyAuthHeaders(options) {
		...
		async function fetchJsonWithTimeout(resource, options, timeoutMs, signal) {
			...
			const mergedOptions = Object.assign({}, applyAuthHeaders(options), {
		...
		function openTokenModal(customDesc) {
			const modalBackdrop = document.getElementById('tokenModalBackdrop');
			const modalInput = document.getElementById('tokenModalInput');
			const modalDesc = document.getElementById('tokenModalDesc');
			if (!modalBackdrop) return;

			if (modalDesc) {
				modalDesc.innerHTML = customDesc || '若 Cloudflare Worker 部署了 <code>TOKEN</code> 环境变量，请在此填入 Token 凭证以授权检测接口。Token 会保存在浏览器本地（localStorage）。';
			}
			if (modalInput) {
				modalInput.value = getSavedToken();
			}
		...
		function saveToken(tokenValue) {
			...
			try {
				if (trimmed) {
					localStorage.setItem('cf_proxy_token', trimmed);
				} else {
					localStorage.removeItem('cf_proxy_token');
				}
			} catch (e) {}
		...
		function updateTokenButtonState() {
			...
			if (token) {
				...
				btn.setAttribute('title', 'Token 已配置 (点击修改)');
				btn.setAttribute('aria-label', 'Token 已配置 (点击修改)');
			} else {
		...
		function initTokenManager() {
			try {
				...
				const queryToken = urlParams.get('token') || urlParams.get('key');
				if (queryToken) {
					localStorage.setItem('cf_proxy_token', queryToken.trim());
			...
			const modalCancelBtn = document.getElementById('tokenModalCancelBtn');
			...
			if (modalCloseBtn) modalCloseBtn.addEventListener('click', closeTokenModal);
			if (modalCancelBtn) modalCancelBtn.addEventListener('click', closeTokenModal);
			if (modalClearBtn) modalClearBtn.addEventListener('click', clearToken);
			if (modalSaveBtn && modalInput) {
				modalSaveBtn.addEventListener('click', function () {
					saveToken(modalInput.value);
				});
			}
			if (modalInput) {
	...
	<!-- Token 配置模态弹窗 -->
	<div class="token-modal-backdrop" id="tokenModalBackdrop" aria-hidden="true">
		<div class="token-modal-card" role="dialog" aria-labelledby="tokenModalTitle">
			<div class="token-modal-header">
				<h3 class="token-modal-title" id="tokenModalTitle">
					<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
						<path d="M21 2l-9.6 9.6"></path>
						<circle cx="7.5" cy="16.5" r="5.5"></circle>
						<path d="m15.5 7.5 3 3"></path>
				...
				<button class="token-modal-close" type="button" id="tokenModalCloseBtn" aria-label="关闭">
					<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
			...
			<p class="token-modal-desc" id="tokenModalDesc">若 Cloudflare Worker 部署了 <code>TOKEN</code> 环境变量，请在此填入 Token 凭证以授权检测接口。Token 会保存在浏览器本地（localStorage）。</p>
			<div class="token-modal-input-wrap">
				<input class="token-modal-input" type="password" id="tokenModalInput" placeholder="输入鉴权 Token..." autocomplete="off" spellcheck="false">
			</div>
			<div class="token-modal-actions">
				<button class="token-modal-btn token-modal-btn-clear" type="button" id="tokenModalClearBtn">清除</button>
